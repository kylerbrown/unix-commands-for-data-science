...
\subsection{globbing}
{\tt ls *.csv} \hfill lists files ending with ``.csv''\\
{\tt ls d*.csv} \hfill lists files starting with ``d'' and ending with ``.csv''\\
{\tt ls data????} \hfill lists files starting with ``data.'' followed by any 4 \\
	\hfill characters\\
{\tt rm [a-z]*[0-9]} \hfill removes files starting with a letter and ending\\
	\hfill in a digit \\
...
{\tt sed \q s/,/ /g\q\ | wc -w data.csv} \hfill {\bf w}ord {\bf c}ount\\
...
source: \url{https://www.overleaf.com/read/nmmwqpvpyvws}\\
